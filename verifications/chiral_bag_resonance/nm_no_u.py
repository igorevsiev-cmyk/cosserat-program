#!/usr/bin/env python3
"""
nm_no_u.py — u-channel disabled control (paper §4.3).

Re-runs the canonical Nelder-Mead minimization of the *director-only*
Cosserat functional (Frank-Oseen + Skyrme, with the Cosserat u-channel
forced to zero, E_u ≡ 0) over the 3-parameter Hopf ansatz (R_r, R_z, w)
on the dyadic box L = 17 x 33.

Purpose: show that WITHOUT the u-channel the bare mass drops to
m_e ≈ 1030 ≠ 1039 cells (442.5 keV) and the canonical dyadic pattern
2^10 + 2^4 - 1 is destroyed (the +2^4 shell and the topological -1 do
not form). Confirms the u-channel is necessary for the dyadic locking
at mu_c = 2*pi (cf. §4.3, §6.7).

The director constants (K_1, K_2, K_3, c_4) keep their canonical values;
only E_u is removed (E_u = 0 in evaluate_energy).

Output:
    - prints per-iteration progress to stdout
    - saves the optimum and its energy decomposition to
      result_u_channel_disable.json
"""
...
def evaluate_energy(R_r, R_z, w, rr, zz, metric, cfg):
    ...
    with torch.no_grad():
        ...
        E_u = 0.0  # TEST: disabled u-channel coupling
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'result_u_channel_disable.json')
    output = {
